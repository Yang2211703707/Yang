...
import { reactive, ref, toRefs, watchEffect, type Ref } from 'vue';
...
const props = withDefaults(defineProps<{ orderList: orderItem[] }>(), {
  orderList: () => [
    {
      orderCode: '000000000000000000',
      type: 'sales',
      state: 'success',
      trading: 'cash',
      amount: 0,
...
const dataList = reactive(orderList);
...
const formatTrading = (row: orderItem) => {
  let text = '';
  switch (row.trading) {
    case 'cash':
      text = '现金支付';
...
const filterTrading = ref<object>([]);
const filterTradingMethod = (value: string, row: orderItem) =>
  row.trading === value;

watchEffect(() => {
  //   筛选出支付渠道数组
  const reduceArr = orderList.value.reduce(
    (res: Array<string>, curr) =>
      res.includes(curr.trading) ? res : [...res, curr.trading],
    []
  );
  //   组成filters需要的数组
  filterTrading.value = reduceArr.map((item) => {
    let text = '';
    switch (item) {
      case 'cash':
        text = '现金支付';
...
<template>
  ...
  >
    ...
    <el-table-column
      ...
      :filters="filterTrading"
      :filter-method="filterTradingMethod"
    />
    <el-table-column
      prop="payable"
      label="应付"
