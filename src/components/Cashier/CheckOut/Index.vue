...
import type { datalist } from '@/components/Cashier/CheckOut/type/checkout';
...
watchEffect(() => {
  datalist.value = updateDatalist(cart.value);
});
function updateDatalist(target: cartItem[]) {
  const list: datalist[] = [];

  target.forEach((item, index) => {
    list[index] = { ...item, show_price_dialog: false };
  });
...
const priceDialog_currIndex = ref(0);
const priceDialogConfirm = (price: number, count: number, discount: number) => {
  console.log('checkout 组件收到更改后的价格：', price, count, discount);
  cart.value[priceDialog_currIndex.value].temp_price = price;
  cart.value[priceDialog_currIndex.value].count = count;
  cart.value[priceDialog_currIndex.value].discount = discount;
  datalist.value[priceDialog_currIndex.value].show_price_dialog = false;
};
...
<template>
  <div class="checkout">
    ...
    <el-scrollbar class="checkout-section">
      <div class="cart-list">
        <div
          class="item"
        ...
        >
          ...
          <div class="item-info">
            ...
            <div
              ...
              @click="
                () => {
                  datalist[index].show_price_dialog = true;
                  priceDialog_currIndex = index;
                }
              "
            >
            ...
            <PriceDialog
              v-if="item.show_price_dialog"
              v-model:visible="datalist[index].show_price_dialog"
              :item="{
                name: item.name,
                price: item.temp_price,
                count: item.count,
